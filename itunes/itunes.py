...
class iTunes(commands.Cog):
    ...
    async def itunes(self, ctx: commands.Context, country: str, *, query: str):
        ...
        try:
            async with aiohttp.request("GET", base_url, params=params) as response:
        ...
        pages = []
        for i, data in enumerate(results.get("results"), start=1):
            ...
            release_date = datetime.strptime(data.get("releaseDate"), "%Y-%m-%dT%H:%M:%SZ")
            embed.add_field(name="Release Date", value=f"<t:{int(release_date.timestamp())}:R>")
            ...
            embed.add_field(
                name="Track Price",
                value=f"{data.get('trackPrice')} {data.get('currency')}",
            )
            embed.set_footer(text=f"Page {i} of {results.get('resultCount')}")
            pages.append(embed)
